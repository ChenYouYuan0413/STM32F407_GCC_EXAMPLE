...
class FoxGlove2OriginCar:
    ...
    def init_topic(self):
        # 地图发布相关设置
        self.map_pub  = Topic(self.ros, "/map", 'nav_msgs/msg/OccupancyGrid',latch=True)
        self.vel_pub  = Topic(self.ros, '/cmd_vel', 'geometry_msgs/msg/Twist')
        ...
        if os.path.exists(self.image_path):
            ...
            self.map_data = {
                'header': {'stamp': {'sec': Time.now().secs, 'nanosec': Time.now().nsecs}, 'frame_id': 'odom_combined'},
                'info': {
    ...
    def keyboard_listener(self):
        ...
        while True:
            sleep(0.05)
            if self.is_pressed('r') or self.isStartTelemetry:         # 激活键盘控制
        ...
        while True:
            ...
            # 优先回复上位机信号
            self.update_sign()
            if self.is_pressed('p') or self.isOverTelemetry:          # 退出激活键盘控制
                self.isOverTelemetry = False
                self.isStartTelemetry = False
                print("\n已退出键盘监听.")
                print("同时按下[ r ]继续键盘监听.")
            ...
            elif self.is_pressed('t'):
                self.help_tip()
                sleep(0.5)
            
            elif self.is_pressed('m'):
                print("pub map")
                self.map_pub.publish(self.map_data)
            else:
